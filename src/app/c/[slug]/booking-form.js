  ...
  useEffect(() => {
    ...
    async function loadSlots() {
      ...
      try {
        ...
        setSlotsMessage(payload.message || (nextSlots.length ? "" : "Nenhum horario disponivel para esta data."));
      } catch {
        if (!cancelled) {
          setSlots([]);
          setSelectedSlot("");
          setSelectedSlotProfessional("");
          setSlotsMessage("Nao foi possivel carregar os horarios. Tente novamente.");
        }
      } finally {
  ...
  return (
    <form action={createPublicBookingAction} className="rounded-[1.75rem] border border-white/70 bg-[#15120f] p-7 text-white shadow-[0_32px_90px_rgba(20,18,15,0.26)]">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="block md:col-span-2">
          <span className="text-sm font-semibold text-white/75">Horarios disponiveis</span>
          <select value={selectedSlot} onChange={(event) => handleSlotChange(event.target.value)} required disabled={loadingSlots || !slots.length} className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-white/10 px-4 text-sm text-white outline-none disabled:cursor-not-allowed disabled:opacity-55">
            {loadingSlots ? <option className="text-neutral-950">Carregando horarios...</option> : null}
            {!loadingSlots && !slots.length ? <option className="text-neutral-950">Sem horarios disponiveis</option> : null}
